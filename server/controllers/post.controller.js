...
const listNewsFeedVideos = (req, res) => {
  let following = req.profile.following
  following.push(req.profile._id)
  let objectIdArray=[]
  for(var i=0;i<following.length;i++)
  {
    objectIdArray.push(following[i]._id);
  }
//console.log(objectIdArray);
res.json(objectIdArray);
}
...
const addVideo = (req, res) => {
  Post.findByIdAndUpdate(req.body.userId, {$push: {uploadedVideos: req.body.publicId}}, {new: true})
  .exec((err, result) => {
    if (err) {
      return res.status(400).json({
        error: errorHandler.getErrorMessage(err)
...
const like = (req, res) => {
  console.log("On Server req.body.postId "+req.body.postId+"req.body.userId "+req.body.userId+"  req.body.mediaId "+req.body.mediaId);
  if(req.body.postId){
